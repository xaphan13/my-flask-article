...
Всё в `docs/` написано по-русски. Нумерованная серия 01–07 рассчитана на чтение по порядку;
...
| [`docs/06_graph_index.md`](docs/06_graph_index.md) | индекс кодовой базы (оговорки покрытия, готовые запросы) и архитектура по измерениям: связность, когезия, хотспоты, сложность |
| [`docs/07_permissions.md`](docs/07_permissions.md) | разрешения Claude Code: auto-режим и его классификатор, почему падает в этом окружении, allow-правила, диагностика |
| [`docs/setup-and-run.md`](docs/setup-and-run.md) | авторитетный, проверенный отчёт по настройке и разбору проблем, с полными таблицами переменных окружения |
| [`AGENTS.md`](AGENTS.md) / [`AGENTS.ru.md`](AGENTS.ru.md) | соглашения и «грабли» для AI-агентов, работающих с кодом |

> ⚠️ 01–05 и `setup-and-run.md` писались по состоянию более раннего коммита. Там, где рабочая
> копия с тех пор разошлась — сборка DSN в `config.py` и env-файлы, фактически отслеживаемые
> git, — проверенные расхождения зафиксированы в
> [`docs/06_graph_index.md`](docs/06_graph_index.md) §4. Читать этот раздел прежде, чем
> доверять таблицам переменных окружения здесь и в `setup-and-run.md`.
